...
json_object_new(enum json_type o_type, size_t alloc_size, json_object_to_json_string_fn *to_json_string);
...
static int json_object_int_to_json_string(struct json_object *jso, struct printbuf *pb, int level, int flags) {
    ...
    if (JC_INT(jso)->cint_type == json_object_int_type_int64)
        snprintf(sbuf, sizeof(sbuf), "%ld", json_object_get_int(jso));
    else
        snprintf(sbuf, sizeof(sbuf), "%lu", json_object_get_int(jso));
...
static int json_object_double_to_json_string_format(
    struct json_object *jso, struct printbuf *pb, int level, int flags, const char *format) {
...
static int
json_object_double_to_json_string_default(struct json_object *jso, struct printbuf *pb, int level, int flags) {
...
struct json_object *json_object_array_bsearch(const struct json_object *key,
    ...
    assert(json_object_get_type(jso) == json_type_array);
    result = (struct json_object **)array_list_bsearch((const void **)(void *)&key, JC_ARRAY_C(jso)->c_array, sort_fn);
...
int json_object_equal(struct json_object *jso1, struct json_object *jso2) {
    ...
    case json_type_string: {
        return (_json_object_get_string_len(JC_STRING(jso1)) == _json_object_get_string_len(JC_STRING(jso2)) &&
...
static int json_object_copy_serializer_data(struct json_object *src, struct json_object *dst) {
    ...
    // else if ... other supported serializers ...
        ...
        _json_c_set_last_err("json_object_copy_serializer_data: unable to copy unknown serializer data: "
                             "%p\n",
 ...
 */
int json_c_shallow_copy_default(
    json_object *src, json_object *parent, const char *key, size_t index, json_object **dst) {
    switch (src->o_type) {
    case json_type_boolean:
 ...
 */
static int json_object_deep_copy_recursive(struct json_object *src,
    ...
    case json_type_object:
        json_object_object_foreachC(src, iter) {
            ...
            if (!iter.val)
                jso = NULL;
            else if (json_object_deep_copy_recursive(iter.val, src, iter.key, UINT_MAX, &jso, shallow_copy) < 0) {
                json_object_put(jso);
                return -1;
            }
