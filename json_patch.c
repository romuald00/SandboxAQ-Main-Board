...
#define _set_err(_errval, _errmsg)                                                                                     \
    do {                                                                                                               \
...
#define _set_err_from_ptrget(_errval, _fieldname)                                                                      \
    do {                                                                                                               \
        patch_error->errno_code = (_errval);                                                                           \
        patch_error->errmsg = (_errval) == ENOENT ? "Did not find element referenced by " _fieldname " field"          \
...
static int
json_object_array_insert_idx_cb(struct json_object *parent, size_t idx, struct json_object *value, void *priv) {
    int rc;
    int *add = priv;
...
int json_patch_apply(struct json_object *copy_from,
    ...
    if (base == NULL || (*base == NULL && copy_from == NULL) || (*base != NULL && copy_from != NULL)) {
